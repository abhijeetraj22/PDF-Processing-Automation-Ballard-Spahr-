...
def move_and_process_files(yesterday_date):
    ...
    merge_folder = os.path.join(destination_path, 'merge')
    if not os.path.exists(merge_folder):
        os.makedirs(merge_folder)
        log_message("Created 'merge' folder in the destination directory.")
    
    # Create folder
    create_folder(source_path, destination_path, yesterday_date)
    ...
    merge_pdfs_in_subfolders(destination_path, yesterday_date)
...
def merge_pdfs_in_subfolders(root_folder, yesterday_date):
    global sub_dir_path
    merge_folder = os.path.join(root_folder, 'merge')
    ...
    if os.path.exists(root_folder) and os.path.isdir(root_folder):
        ...
        for root, dirs, files in os.walk(root_folder):
            for sub_dir in dirs:
                ...
                # Exclude 'allMergePdf' folder
                if sub_dir != 'allMergePdf':
                    try:
                        # Check if the subdirectory path is not None and is a valid directory
                        if sub_dir_path is not None and os.path.isdir(sub_dir_path):
                            ...
                            # Check if the subdirectory doesn't have a date-month-year format
                            #if not any(part.isdigit() for part in os.path.basename(sub_dir_path).replace('-', '').split()):
                            
                                # Find all PDF files in the subdirectory
                                pdf_files = [f for f in os.listdir(sub_dir_path) if f.lower().endswith('.pdf')]
                                ...
                                if len(pdf_files) == 0 and len(image_files) == 0:
                                    log_message(f"No PDF or image files found in subdirectory '{sub_dir}'. Skipping.")
                                    continue  # Skip to the next subdirectory
                                                                
                                # If subdirectory is 'Log' or 'merge' and no PDF files found
                                if sub_dir in ['Log', 'merge']:
                                    continue
                                ...
                                # Attempt to merge PDFs using different packages
                                    ...
                                    merged_file_path = attempt_merge_pdf_files(pdf_files, sub_dir, sub_dir_path, yesterday_date)
                    ...
                    log_message(f"Skipping processing of subdirectory '{sub_dir}' because it is 'merge' folder.")
    else:
...
def attempt_merge_pdf_files(pdf_files, sub_dir, sub_dir_path, yesterday_date):
...
def create_folder(source_folder, destination_folder, yesterday_date):
    ...
    for item_name in os.listdir(source_folder):
        ...
        # Create 'merge' folder in the root folder if it doesn't exist
        merge_folder = os.path.join(destination_folder, 'merge')
        if not os.path.exists(merge_folder):
            os.makedirs(merge_folder)
            log_message(f"Created folder '{merge_folder}'.")

        # Check if the item is a directory
        if os.path.isdir(source_item_path) or os.path.isfile(source_item_path):
            # Get the modified date of the item
            modified_date = yesterday_date

            if modified_date:
                # Create a folder based on the modified date in the 'merge' folder
                ...
                # Ensure the date folder exists or create it in the 'merge' folder
...
if __name__ == "__main__":
    ...
    # Call the main function with yesterday's date
    move_and_process_files(yesterday_date)
